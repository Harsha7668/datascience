...
async def encode(filepath, message, msg):
   ...
   """ x265 = await db.get_hevc(message.from_user.id)
    if x265:
       ...
       x265 = await db.get_hevc(message.from_user.id)
    if x265:
        video_opts = '-profile:v main10 -pix_fmt yuv420p10le -x265-params "internal-bit-depth=10" -map 0:v? -map_chapters 0 -map_metadata 0'
    else:
        video_opts = f'{cabac} {reframe} -profile:v main10 -pix_fmt yuv420p10le -x265-params "internal-bit-depth=10" -map 0:v? -map_chapters 0 -map_metadata 0'

    
    # Metadata Watermark
    m = await db.get_metadata_w(message.from_user.id)
    if m:
        metadata = '-metadata title=SUNRISES-HARSHA-24 -metadata:s:v title=SUNRISES-HARSHA-24 -metadata:s:s title=HARSHA-24-Sunrises24Rips -metadata:s:a title=HARSHA-24-Sunrises24Rips'
    else:
        metadata = ''
